...
onMounted(() => {
  ...
  gsap.to(elementsToAnimate, {
    opacity: 1, scale: 1, y: 0, duration: 0.8, delay: 0.3, stagger: 0.2, ease: 'power3.out',
  });
});
...
.title-area { padding-top: 1rem; }
.feed-title-container { display: flex; justify-content: center; align-items: center; gap: 1.5rem; margin-bottom: 1.5rem; }
.title-background-container { display: inline-block; position: relative; border-radius: 25px; padding: 0.3rem 1.5rem; overflow: hidden; z-index: 1; border: 2px solid rgba(255, 255, 255, 0.2); animation: animated-border-glow 4s linear infinite; transition: box-shadow 0.5s ease; }
.title-background-container::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: url('/fondoInfo.jpg'); background-size: cover; background-position: center; opacity: 0.9; z-index: -1; }
.stream-fade-enter-active, .stream-fade-leave-active { transition: all 0.5s ease; }
.stream-fade-enter-from, .stream-fade-leave-to { opacity: 0; transform: translateY(-20px); }
.feed-title { font-family: 'Uncial Antiqua', serif; text-align: center; font-size: 2rem; color: #fca311; transition: text-shadow 0.5s ease; position: relative; z-index: 2; }
.fenicia-subtitle { font-family: 'Playfair Display', serif; font-style: italic; font-weight: 500; color: white; text-align: center; font-size: 0.9rem; margin-bottom: 1rem; text-shadow: 0 0 7px black, 0 0 4px black; }
.inline-icon { height: 1.7em; width: auto; vertical-align: text-bottom; margin: 0 0.2em; }
.stream-area { margin-bottom: 1rem; aspect-ratio: 16 / 9; }
.chat-area, .posts-area { margin-bottom: 1rem; }
.chat-area { position: relative; max-height: 70vh; display: flex; flex-direction: column; }
.pin-banners-container { position: absolute; top: 0; left: 0; right: 0; z-index: 10; padding: 1rem; pointer-events: none; }
.pin-banners-container > :deep(*) { pointer-events: auto; }
.chat-area > :deep(.global-chat-container) { flex-grow: 1; min-height: 0; }
.posts-area { padding-bottom: 1rem; }
.loading-spinner { text-align: center; font-size: 1.5rem; padding: 2rem; color: #999; }
.fade-in-item { opacity: 0; transform: scale(0.95) translateY(20px); }
.load-trigger { height: 50px; }
.stream-area.fade-in-item { opacity: 1 !important; transform: none !important; }
@media (max-width: 991px) { .feed-title-container { flex-direction: column; gap: 1rem; align-items: center; } .feed-title { font-size: 1.6rem; } .title-background-container { padding: 0.2rem 1rem; } }
@media (min-width: 992px) { .social-feed-layout { display: grid; grid-template-columns: minmax(0, 1.5fr) minmax(0, 1fr); grid-template-rows: auto auto 1fr; grid-template-areas: 'title chat' 'stream chat' 'posts chat'; gap: 1.5rem; max-width: 1600px; margin: 0 auto; padding: 1rem 2rem 0 180px; height: calc(100vh - 80px); min-height: auto; } .title-area { grid-area: title; padding-top: 0; } .stream-area { grid-area: stream; } .chat-area { grid-area: chat; height: 100%; max-height: none; } .posts-area { grid-area: posts; overflow-y: auto; padding-right: 1rem; } .posts-area::-webkit-scrollbar { width: 8px; } .posts-area::-webkit-scrollbar-track { background: transparent; } .posts-area::-webkit-scrollbar-thumb { background-color: #4a525d; border-radius: 10px; border: 2px solid transparent; background-clip: content-box; } .feed-title-container { position: sticky; top: 0; flex-direction: row; background-color: transparent; z-index: 10; padding: 1rem 0; } }
</style>