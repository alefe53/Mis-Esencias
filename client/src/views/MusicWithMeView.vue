<template>
	<div class="music-with-me-view">
		...
		<div class="main-area">
			...
			<div v-else-if="!bandStore.isLoading && bandStore.bands.length === 0" class="empty-state">
				...
				<div class="bands-list">
					<div v-for="band in bandStore.bands" :key="band.id" class="band-item">
						<div class="band-header" @click="toggleBand(band.id)">
							...
							<span class="chevron" :class="{ expanded: expandedBandId === band.id }">&#9660;</span>
						</div>
...
.expand-enter-active,
.expand-leave-active {
	transition: grid-template-rows 0.4s ease, opacity 0.4s ease-in-out;
