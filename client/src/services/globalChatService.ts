...
export const postMessage = async (content: string, parentId: number | null, wantsToPin: boolean): Promise<GlobalChatMessage> => {
  const response = await api.post('/global-chat', { content, parentId, wantsToPin });
  return response.data.data;
};
