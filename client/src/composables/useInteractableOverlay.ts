...
export function useInteractableOverlay(
  ...
  const onMouseMove = (e: MouseEvent) => {
    ...
    if (isDragging) {
      ...
      newX = Math.max(
        0,
        Math.min(newX, parentRect.width - target.value.offsetWidth),
      )
      newY = Math.max(
        0,
        Math.min(newY, parentRect.height - target.value.offsetHeight),
      )
    ...
    if (isResizing) {
      ...
      const maxWidthPx =
        parentRect.width - (options.position.value.x / 100) * parentRect.width
  ...
  const cleanupListeners = () => {
    if (target.value)
      target.value.removeEventListener('mousedown', onMouseDownTarget)
    if (resizeHandle.value)
      resizeHandle.value.removeEventListener('mousedown', onMouseDownResize)
    document.removeEventListener('mousemove', onMouseMove)
    document.removeEventListener('mouseup', onMouseUp)
  }
  ...
  watch(target, (newTarget, oldTarget) => {
    ...
    if (newTarget) {
      ...
      if (resizeHandle.value)
        resizeHandle.value.addEventListener('mousedown', onMouseDownResize)
    }
  })
