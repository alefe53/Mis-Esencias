...
async function attachTrackToEl(track: LiveKitTrack | null) {
  ...
  try {
    track.attach(el)
    console.log(
      `[ParticipantViewV2] -> ✅ Attach inicial de track ${track.sid}.`,
    )
    ...
    if (track.kind === 'video' && el instanceof HTMLVideoElement) {
      ...
      setTimeout(() => {
        if (el.videoWidth === 0 && el.videoHeight === 0) {
          console.warn(
            `[ParticipantViewV2] -> ⚠️ No se detectan frames en el video ${track.sid}. Reintentando attach...`,
          )
          ...
          console.log(
            `[ParticipantViewV2] -> ✨ Frames detectados para ${track.sid} (${el.videoWidth}x${el.videoHeight}).`,
          )
        }
    ...
    console.warn(
      `[ParticipantViewV2] -> ⚠️ Attach del track ${track.sid} falló:`,
      err,
    )
  }
}
