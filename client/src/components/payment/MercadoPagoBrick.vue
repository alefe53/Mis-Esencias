...
const initializeBrick = async () => {
  if (typeof window.MercadoPago !== 'undefined') {
    ...
    console.log(
      '🧱 [MercadoPagoBrick] Intentando inicializar con Public Key:',
      publicKey,
    )

    if (!publicKey || !publicKey.startsWith('TEST-')) {
      console.error(
        '🔥🔥🔥 ¡ERROR CRÍTICO! La Public Key que está usando el código NO es de prueba. Valor actual:',
        publicKey,
      )
      // Lanzamos una alerta para que sea imposible de ignorar
      alert(
        `ERROR DE CONFIGURACIÓN: La Public Key de Mercado Pago no es de prueba. El valor que se está usando es: ${publicKey}`,
      )
    } else {
