...
export const usePostStore = defineStore('posts', () => {
    ...
    async function toggleCommentLike(postId: number, commentId: number) {
        ...
        try {
            ...
            useUiStore().showToast({ message: "No se pudo registrar el Me Gusta.", color: '#EF4444' });
        }
    }
    ...
    async function castVote(postId: number, optionId: number) {
        ...
        if (post.pollData.userVote === null) {
            ...
            const votedOption = post.pollData.options.find(opt => opt.id === optionId);
    ...
    async function updatePost(postId: number, payload: { title: string; content: string }) {
        const uiStore = useUiStore();
        try {
            ...
            uiStore.showToast({ message: "Publicación actualizada.", color: '#4ade80' });
        } catch (error) {
    ...
    async function updateComment(postId: number, commentId: number, payload: { content: string }) {
        const uiStore = useUiStore();
        try {
            const updatedCommentData = await adminService.updateComment(commentId, payload);
            const post = posts.value.find(p => p.id === postId);
            if (post && post.comments) {
            ...
            uiStore.showToast({ message: "Comentario actualizado.", color: '#4ade80' });
        } catch (error) {
