	...
	actions: {
		async fetchBandsWithReleases() {
            ...
            try {
                ...
                console.log('1. DATOS CRUDOS RECIBIDOS:', JSON.parse(JSON.stringify(fetchedBands)));
                ...
                fetchedBands.sort((bandA, bandB) => {
                    ...
                    console.log(`2. COMPARANDO: ${bandA.name} (año ${latestYearA}) vs ${bandB.name} (año ${latestYearB})`);
                    
                    return latestYearB - latestYearA;
                });

                // 3. Miremos cómo quedó el array DESPUÉS de intentar ordenarlo
                console.log('3. ARRAY DESPUÉS DE ORDENAR:', JSON.parse(JSON.stringify(fetchedBands)));
