...
const sortedTiers = computed(() => tiers.value.sort((a, b) => a.price - b.price))
...
async function createMercadoPagoPreference(tierId: number, durationMonths: number) {
...
const message = error.response?.data?.message || 'Error al crear la preferencia de Mercado Pago.'
...
const message = error.response?.data?.message || 'Error al crear la orden de PayPal.'
paymentError.value = message
console.error("Error detallado:", error.response?.data || error)
return null
}
...
paymentError.value = 'No se pudo completar el pago. Por favor, intenta de nuevo.'
console.error(error)
uiStore.showToast({
