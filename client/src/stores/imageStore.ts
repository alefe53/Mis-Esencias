...
export const useImageStore = defineStore('images', () => {
    ...
    const wasToastViewed = ref(localStorage.getItem('galleryToastViewed') === 'true');
    ...
    const currentIndex = computed(() => {
        ...
        const selectedKey = 'imageUrl' in selectedImage.value ? selectedImage.value.imageUrl : selectedImage.value.file_path;
    ...
    const hasNext = computed(() => currentIndex.value >= 0 && currentIndex.value < galleryItems.value.length - 1);
    ...
    function selectImage(image: PrivateGalleryImage) {
        ...
        const startIndex = privateGallery.value.findIndex(img => img.file_path === image.file_path);
        if (startIndex !== -1) {
    ...
    function openLightbox(images: (GalleryImage | PrivateGalleryImage)[], startIndex: number) {
