...
} from '../composables/streaming/useStreamStateV2'
...
export const useStreamingStoreV2 = defineStore('streamingV2', () => {
  ...
  function broadcastWithRetries(times = [250, 800, 2000]) {
    times.forEach((delay, idx) => {
      setTimeout(() => {
        console.log(
          `[STORE-BROADCAST-RETRY] -> Intento ${idx + 1} (delay ${delay}ms)`,
        )
  ...
  const setupRoomListeners = (newRoom: Room) => {
    newRoom
      .on(RoomEvent.ParticipantConnected, () => {
        console.log(
          '👤 [STORE-EVENT] Un espectador se ha conectado. Enviando estado actual del layout...',
        )
      ...
      .on(RoomEvent.DataReceived, (payload, participant) => {
        try {
          ...
          if (data?.type === 'request_layout') {
            console.log(
              `[STORE-EVENT] -> Petición de layout recibida de ${participant?.identity}. Respondiendo...`,
            )
      ...
      .on(RoomEvent.LocalTrackPublished, (pub: TrackPublication) => {
        ...
        if (pub.source === Track.Source.ScreenShare)
          _writableState.isScreenSharing = true
      })
      .on(RoomEvent.LocalTrackUnpublished, (pub: TrackPublication) => {
        ...
        if (pub.source === Track.Source.ScreenShare)
          _writableState.isScreenSharing = false
      })
  ...
  async function restartCameraTrack() {
    ...
    if (!cameraTrack || !(cameraTrack instanceof LocalVideoTrack)) {
      console.warn(
        '[STORE-RESTART-CAM] -> ⚠️ No se encontró un track de video local para reiniciar.',
      )
      return
    }

    console.log(
      '[STORE-RESTART-CAM] -> 🚀 Iniciando reinicio de track de cámara.',
    )
    try {
      // ¡Este es el método correcto! Se llama sobre el track.
      await cameraTrack.restartTrack(cameraOptions)
      console.log(
        '[STORE-RESTART-CAM] -> ✅ Track de cámara reiniciado exitosamente.',
      )
    } catch (e) {
      console.error(
        '[STORE-RESTART-CAM] -> ❌ Error reiniciando el track de cámara:',
        e,
      )
    }
  }
  ...
  async function toggleScreenShare() {
    ...
    try {
      ...
      console.error(
        '[STORE] -> ❌ Error en toggleScreenShare. Revirtiendo estado.',
        e,
      )
      _writableState.isScreenSharing = !newState
    } finally {
  ...
  function cycleCameraOverlayPosition() {
    const p: OverlayPosition[] = [
      'bottom-left',
      'top-left',
      'top-right',
      'bottom-right',
    ]
    const i = p.indexOf(streamState.cameraOverlay.position)
    _writableState.cameraOverlay.position = p[(i + 1) % p.length]
  }
  function toggleCameraFocus() {
    _writableState.cameraOverlay.isCameraFocus =
      !_writableState.cameraOverlay.isCameraFocus
  }
  async function getPermissionsAndPreview() {
    if (previewTrack.value) return
    try {
      const track = await createLocalVideoTrack(cameraOptions)
      previewTrack.value = track
    } catch (e) {
      _writableState.permissionError = 'Permiso denegado.'
    }
  }
  async function enterStudio() {
    ...
    try {
      const res = await api.get('/streaming/token')
      const newRoom = new Room({ adaptiveStream: true, dynacast: true })
      setupRoomListeners(newRoom)
      await newRoom.connect(import.meta.env.VITE_LIVEKIT_URL, res.data.token)
      room.value = newRoom
  ...
  async function publishMedia() {
    if (
      ...
      streamState.isPublishing !== 'inactive' ||
      !previewTrack.value
    )
      return
