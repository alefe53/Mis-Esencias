...
export const useGlobalChatStore = defineStore('globalChat', () => {
    ...
    return messages.value
      .filter(
        (m) =>
          m.is_pinned &&
          m.pinned_until &&
          new Date(m.pinned_until).getTime() > now,
      )
      .sort(
        (a, b) =>
  ...
  async function fetchOlderMessages() {
    ...
    try {
      const oldestMessage = allSortedMessages.value[0]
      const cursor = oldestMessage
        ? oldestMessage.created_at
        : new Date().toISOString()

      const olderMessages = await globalChatService.fetchHistory(25, cursor)

      if (olderMessages.length > 0) {
        const uniqueMessages = olderMessages.filter(
          (om) => !messages.value.some((em) => em.message_id === om.message_id),
        )
  ...
  async function connect() {
    ...
    try {
      const initialMessages = await globalChatService.fetchHistory(25)
      messages.value = initialMessages.reverse()
      if (initialMessages.length < 25) {
        hasMoreHistory.value = false
      }
    } catch (e) {
    ...
    realtimeChannel
      .on('broadcast', { event: 'new_message' }, async (payload) => {
        ...
        const existing = messages.value.find(
          (m) => m.message_id === newMessageId,
        )
        if (existing) return
        const fullMessage =
          await globalChatService.fetchSingleMessage(newMessageId)
        ...
        (payload) => {
          ...
          if (deletedMessageId) {
            messages.value = messages.value.filter(
              (msg) => msg.message_id !== deletedMessageId,
            )
          }
        },
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'global_chat_reactions' },
        async (payload) => {
          ...
          try {
            const updatedMessage =
              await globalChatService.fetchSingleMessage(messageIdToUpdate)
            if (updatedMessage) {
            ...
            console.error(
              `Error al refrescar mensaje ${messageIdToUpdate} tras reacción:`,
              error,
            )
          }
        },
  ...
  async function sendMessage(
    ...
    try {
      ...
      if (
        wantsToPin &&
        authStore.user?.subscription_tier_id === 3 &&
        !authStore.isAdmin
      ) {
