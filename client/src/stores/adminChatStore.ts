...
export const useAdminChatStore = defineStore('adminChat', () => {
  ...
  async function selectConversation(convo: AdminConversationSummary) {
    ...
    try {
      currentMessages.value = await adminService.getConversationDetailsAsAdmin(
  ...
  async function handleReply(content: string) {
    ...
    try {
      ...
      const convoInList = conversations.value.find(
        (c) => c.conversation_id === payload.conversationId,
      )
      if (convoInList) {
    ...
    realtimeChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const convoInList = conversations.value.find(
            (c) => c.conversation_id === newMessage.id,
          )
          if (convoInList) {
  ...
  async function deleteConversation(conversationId: number) {
    const confirmation = window.confirm(
      '¿Estás seguro de que quieres borrar esta conversación PERMANENTEMENTE?',
    )
