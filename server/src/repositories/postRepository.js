...
const getFeedFromDB = async (_authToken, userId, page, limit) => {
...
const deletePostFromDB = async (authToken, postId) => {
	const scopedSupabase = createScopedClient(authToken);
	const { error } = await scopedSupabase.rpc("admin_delete_post", {
		p_post_id: postId,
	});
	if (error) {
...
const deleteCommentFromDB = async (authToken, commentId) => {
	const scopedSupabase = createScopedClient(authToken);
	const { error } = await scopedSupabase.rpc("admin_delete_comment", {
