...
const getChatMessages = async (req, res, next) => {
	try {
		...
		const messages = await globalChatService.fetchMessages(
			parseInt(limit, 10),
			cursor,
		);
...
const postChatMessage = async (req, res, next) => {
	try {
		const { content, parentId, wantsToPin = false } = req.body;
		const authToken = extractAuthToken(req);
		const newMessage = await globalChatService.postMessage(
			authToken,
			content,
			parentId,
			wantsToPin,
...
const toggleMessageReaction = async (req, res, next) => {
	try {
		...
		const authToken = extractAuthToken(req);
		const updatedReactions = await globalChatService.toggleReaction(
			authToken,
			messageId,
			emoji,
		);
...
const getSingleChatMessage = async (req, res, next) => {
	try {
		const { messageId } = req.params;
		const message = await globalChatService.fetchSingleMessage(
