...
const getSocialFeed = async (req, res, next) => {
    try {
        ...
        const feed = await postService.fetchSocialFeed(authToken, userId, page, limit); 
...
const addComment = async (req, res, next) => {
    try {
        ...
        const authToken = req.headers.authorization.split(" ")[1];
        const userId = req.user.id;
        const newComment = await postService.addComment({ postId, content, imageUrl, parentId }, authToken, userId);
...
const castVote = async (req, res, next) => {
  try {
    ...
    if (!optionId) {
      return res.status(400).json({ success: false, message: "optionId es requerido." });
    }
    
    const updatedPollData = await postService.castPollVote(authToken, postId, optionId);
