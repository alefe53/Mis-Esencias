...
const fetchSocialFeed = async (authToken, userId, page = 1, limit = 10) => {
	const feedData = await postRepository.getFeedFromDB(
		authToken,
		userId,
		page,
		limit,
	...
	return Promise.all(
		feedData.map(async (item) => {
			...
			if (post.author.avatarUrl) {
				post.author.avatarUrl = getPublicUrl(
					PUBLIC_BUCKET,
				...
				post.comments = await Promise.all(
					post.comments.map(async (comment) => {
						if (comment.author.avatarUrl) {
							comment.author.avatarUrl = getPublicUrl(
								PUBLIC_BUCKET,
								comment.author.avatarUrl,
							);
						}
						if (comment.imageUrl) {
							comment.imageUrl = await createPrivateUrl(
								PRIVATE_BUCKET,
								comment.imageUrl,
							);
						}
...
const addComment = async (
	...
	const newComment = await postRepository.addCommentToDB(
		authToken,
		userId,
		postId,
		content,
	...
	if (newComment.author.avatarUrl) {
		newComment.author.avatarUrl = getPublicUrl(
			PUBLIC_BUCKET,
			newComment.author.avatarUrl,
		);
	}
	if (newComment.imageUrl) {
		newComment.imageUrl = await createPrivateUrl(
			PRIVATE_BUCKET,
			newComment.imageUrl,
		);
	}
