...
export const getPrivateGalleryImages = async (galleryKey, userAuthToken) => {
	...
	const imagesData = await imageRepository.getGalleryByKeyFromDB(
		galleryKey,
	...
	if (!signedUrlsData) {
		throw new Error(
			"No se pudieron generar las URLs seguras para las imágenes.",
		);
	}

	return imagesData.map((imageData) => {
		const signedUrlItem = signedUrlsData.find(
			(item) => item.path === imageData.file_path,
		);
		return {
			...imageData,
